    ...
    switch (keycode) {
        case LOWER_TAP:
            if (record->event.pressed) {
                ...
                if (timer_elapsed(lower_timer) < 150) {
                    // Short press = tap
                    if (lower_tap_pending && timer_elapsed(lower_tap_timer) < 225) {
        ...
        case RAISE_TAP:
            if (record->event.pressed) {
                ...
                // Key released - check if it was a tap or hold
                if (timer_elapsed(raise_timer) < 150) {
                    // Short press = tap
                    if (raise_tap_pending && timer_elapsed(raise_tap_timer) < 225) {
...
void matrix_scan_user(void) {
    ...
    if (lower_held && timer_elapsed(lower_timer) > 150) {
        uint8_t lower_layer = (current_layer_set * 4) + 1;  // Base + 1 = lower layer
        layer_on(lower_layer);
        if (raise_held && timer_elapsed(raise_timer) > 150) {
...
void my_render_mod_status_gui_alt(uint8_t modifiers) {
    static const char PROGMEM gui_off_1[] = {0x9f, 0xa0, 0};
    static const char PROGMEM gui_off_2[] = {0xbf, 0xc0, 0};
    static const char PROGMEM gui_on_1[] = {0x9d, 0x9e, 0};
    static const char PROGMEM gui_on_2[] = {0xbd, 0xbe, 0};
...
void my_render_layer_state(void) {
    ...
    switch (get_highest_layer(layer_state | default_layer_state)) {
        ...
        case 6: // Nordic Raise
            ...
            break;
        case 3: // Adjust
        case 7: // Nordic Adjust
            oled_write_P(adjust_layer, false);
            break;
        default: // Default and Nordic base
